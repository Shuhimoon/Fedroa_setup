...
install_fonts(){
	...
	local install_fronts=("jetbrains-mono-fonts")

	echo " 檢查套件是否已安裝..."

	for cf in "${copr_fonts[@]}"; do
        if ! dnf repolist all | grep -q "$cf"; then
            echo "OK!  $cf 已安裝"
        else
            echo "$cf 未安裝，快去安裝!!!"
            echo "開啟copr $cf  !!"
            sudo dnf copr enable "$cf" -y
            for ins_fonts in "${install_fronts[@]}";do
                sudo dnf install "$ins_fonts" -y
        fi
    done
}
...
install_xrdp() {
    local conntion_packages=("xrdp" "tigervnc-server")
	  local install_conntion=()
	  local user_add=("shuhi" "fancie")

	  echo " 檢查套件是否已安裝..."

	  for cpkg in "${conntion_packages[@]}"; do
        if command -v "$cpkg" >/dev/null 2>&1; then
            echo "୨୧  OK!  $cpkg 已安裝  ୨୧"
        else
            echo " ૮₍ꐦ-᷅ ⤙ -᷄  ₎  $cpkg 未安裝，快去安!!!"
            to_install+=("$cpkg")

            echo "是否要安裝 xrdp 和 VNC？ (y/n)"
            read -p "" choice
            # 轉小寫以處理大小寫不敏感
            choice=${choice,,}
            ...
            if [[ "$choice" == "y" || "$choice" == "yes" ]]; then
                ...
                # 安裝 xrdp 及相關套件（包含 tigervnc-server 以支援 VNC 後端）
                sudo dnf install -y "$ckpg"
                ...
                if ["$cpkg" == "xrdp"] ;then
                    ...
                    echo "請輸入要設定的使用者名稱列表（以逗號分隔，例如: shuhi,fanice）："
                    read -p "" user_input
                    ...
                    for user in "${users[@]}"; do
                        ...
                        sudo firewall-cmd --permanent --add-rich-rule="rule family=\"ipv4\" port port=\"$port\" protocol=\"tcp\" accept"
